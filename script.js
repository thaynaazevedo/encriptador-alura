...
document.getElementById('btn-criptografar').onclick = (e) => {
  ...
  if (input.value) {
    mensagem.value = encriptar(input.value.toLowerCase());
  } else if (mensagem.value) {
    mensagem.value = encriptar(mensagem.value.toLowerCase());
  } else {
    mensagem.value = encriptar(input.value.toLowerCase());
  }
...
document.getElementById('btn-descriptografar').onclick = (e) => {
  ...
  if (input.value) {
    mensagem.value = decodificar(input.value.toLowerCase());
  } else if (mensagem.value) {
    mensagem.value = decodificar(mensagem.value.toLowerCase());
  } else {
    mensagem.value = decodificar(input.value.toLowerCase());
  }
